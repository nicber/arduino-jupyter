...
check('la velocidad sale en rad/s',
      np.allclose(_w, 2 * np.pi) and len(_w) == len(_t) - 1, str(_w[:3]))
check('el promedio no cambia una velocidad constante',
      np.allclose(ensayo.velocidad(_df, ventana=0.05)[1], 2 * np.pi))
...
with tempfile.TemporaryDirectory() as _tmp:
    _ruta = ensayo.guardar(_df, Path(_tmp) / 'sub' / 'e.csv', ventana=0)
