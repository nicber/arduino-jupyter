...
class FreeAdc
{
    ...
    // Ver board::adc_select_reference().
    //
    // Sin `internal_ref`, contra Vcc. Un sensor de corriente bipolar y ratiométrico
    // --un ACS712-- reposa en la mitad de su alimentación, y medirlo contra la misma
    // tensión que lo alimenta lo deja en media escala por construcción.
    void begin(uint16_t full_scale, uint16_t normalized, bool internal_ref = false)
    {
        m_shift = 0;
